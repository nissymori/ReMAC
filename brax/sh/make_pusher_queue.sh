...
# per line, for sh/queue_worker.sh to consume.
#
...
for tag in m1 m2 m4 m8 sac ppo; do
  echo "PUSHER_LR=$LR bash sh/pusher_final.sh \$GPU \"$tag\""
done

# 2. the diagnostics tables: B = 16 at every M, Adam and SGD actors
for m in 1 2 4; do
  echo "PUSHER_LR=$LR bash sh/pusher_sigma_sgd.sh \$GPU \"adam:$m\""
done
for m in 1 2 4; do
  echo "PUSHER_LR=$LR bash sh/pusher_sigma_sgd.sh \$GPU \"sgd:$m\""
done

# 3. the epsilon sweep (eps = 1e-8 is already covered by the main runs)
for eps in 1e-2 1e-1 1; do
  for m in 1 2 4 8; do
    echo "PUSHER_LR=$LR bash sh/pusher_eps.sh \$GPU \"$m:$eps\""
  done
done
